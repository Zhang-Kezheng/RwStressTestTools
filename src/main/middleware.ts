...
import { IotBoxMiddlewareGatewayVo, IotBoxTagVo, IotBoxMiddlewareGateway } from '../common/vo'
...
export async function start(
  ...
  const cache_map: Map<string, IotBoxTagVo[]> = new Map<string, IotBoxTagVo[]>()
  const timer = setInterval(() => {
    gateway_list.forEach((item) => {
      ...
      cache_map.forEach((value, key) => {
        ...
        fs.appendFileSync(path.join(app.getPath('userData'), 'cache', `${key}.cache`), data)
      })
      cache_map.clear()
    })
  }, 1000)
  return new Promise<boolean>((resolve, reject) => {
    ...
    server.message(async (message) => {
      try {
        const [mac, tag_list, tag_map] = await pool.exec('process', [message])
        const tags: IotBoxTagVo[] = JSON.parse(JSON.stringify(tag_list))
...
export async function export_tag_list(
  ...
  if (option.export_mode == 0) {
    ...
    //合并导出
    fs.appendFileSync(
      path.join(option.path, `${option.gateway_mac}.csv`),
    ...
    const data = fs
      .readFileSync(path.join(app.getPath('userData'), 'cache', `${option.gateway_mac}.cache`))
      .toString()
    const list: IotBoxTagVo[] = []
    data.split(`${os.EOL}`).forEach((item) => {
      const protocol = IotBoxTagProtocol.getInstance(hexStringToByteArray(item).buffer)
      if (protocol != null) {
        const vo = IotBoxTagVo.transform(protocol, option.gateway_mac)
        list.push(vo)
      }
    })
    list
